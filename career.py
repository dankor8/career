...
def newTable(headers, data, title = None):
    table = Table(title = title)
    for column in headers:
        if isinstance(column, str):
            column = [column, 'left']
        table.add_column(header = richFormat(str(column[0])), justify = column[1])
    for row in data:
        table.add_row(*[richFormat(str(x)) for x in row])
    return table
...
def createNations():
    ...
    for i, nationData in enumerate(nationsData, 1):
        nationNames, nationNationality, nationColor, nationCode = nationData[:-1].split(' + ')
        nationNames = nationNames.split(', ')
        allNationNames += nationNames
        Nation(nationNames, nationCode, nationNationality, nationColor, i)
...
def viewClubStandings():
    tableColumns = ['№', ['Rating', 'center'], ['Nation', 'center'], 'Colors', 'Generic name', 'Full name', 'Nickname']
    tableRows = []
    ...
    richPrint(newTable(tableColumns, tableRows, 'Worldwide football club rankings:'))
    input('\n<yellow>Press Enter to go back to the start menu: </yellow>')
...
def importPackages():
    ...
    global sha3_224, random, shuffle, choice, choices, safe_load, safe_dump, Layout, Table, Console, Text
...
print('\n'.join([colorText(code, code) for code in mainStyleDict.keys()]))
input()
