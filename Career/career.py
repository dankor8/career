...
def richFormat(text: str) -> str:
    '''
    Formats HTML formatted text to match Rich formatting.
    '''
...
def colorSettingValue(value: str, mode: str = 'html') -> str | list | None:
    ...
    If mode is 'html' (default), returns the value that's colored using prompt_toolkit's HTML format.
    '''
    match mode:
        case 'html':
            if value.lower() == 'yes':
...
def isValidChangeDate(file: str, changeDate) -> bool:
    '''
    Checks if the changeDate in the file matches it's system-coded change date.
    '''
...
def saveSetup() -> None:
    '''
    Saves a Hero object taken from the hero variable into a file.
    '''
    clear()
    while True:
...
def yesNoMenu(text, default = None) -> bool:
    '''
    Creates a simple Yes or No menu.\n
...
def floor(num) -> int:
    '''
    Rounds the number to the lowest integer of the two nearest ones.
    '''
    return int(num // 1)
