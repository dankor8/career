...
def viewLeagueRankings():
    ...
    for i, league in enumerate(sorted(League.instances, key = lambda x: -x.rating), 1):
        tableRows.append([i, league.rating, league.nation.shortName + league.nation.colorText(league.level), league.name, ', '.join([club.name for club in league.sortedClubs[:3]])])
...
def importPackages():
    ...
    global getmtime, exists, fixWin, questionary, Choice, sysExit, datetime
...
class Nation:
    ...
    @staticmethod
    def find(name, allowExceptions = False):
        name = str(name).lower()
        for nation in Nation.instances:
...
class Player:
    ...
    def viewProfile(self):
        ...
        clear()
        print(f'<bold>{(self.fullName + f"{singleQuote}s profile:").center(35 + len(self.fullName) - len(self.ucFullName))}</bold>\n')
...
class League:
    ...
    def __init__(self, name, nation, clubs):
        ...
        self._searchOptions = [self.ucName.lower(), self.nation.ucShortName.lower() + str(self.level)] + [f'{nationName.lower()} {self.level}' for nationName in self.nation.ucNames]
