...
# use tables (for rankings and other stuff)
# add nation rankings
# add league rankings
# group all rankings into 1 button
...
def createNations():
    ...
    for i, nationData in enumerate(nationsData, 1):
        nationNames, nationNationality, nationColor = nationData[:-1].split(' + ') #add nationCode!!! TODO
        nationNames = nationNames.split(', ')
        allNationNames += nationNames
        Nation(nationNames, 'aaa', nationNationality, nationColor, i)
...
def viewClubStandings():
    ...
    tableRows = []
    for i, club in enumerate(sorted(Club.instances, key = lambda x: -x.ovr), 1):
        tableRows.append([i, club.ovr, club.nation.shortName, club.colorText('   ', bg = True) + club.color2Text('   ', bg = True), club.name, club.fullName, club.nickname])
    ...
    def getPrintable(self):
        ...
        for row in self.data:
            for columnLen, header, cell, uncoloredCell in zip(columnLens, self.headers, row.cells, row.uncoloredCells):
                table += self.style['dataVert'] + self.style['fill'] + alignText(colorText(cell, header.rowColor), uncoloredCell, header.rowAlign, columnLen, fill = self.style['fill']) + self.style['fill']
